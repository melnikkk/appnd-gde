...
import { MediaProcessor } from '../interfaces/media-processor.interface';
import { PathManager } from '../../storage/interfaces/path-manager.interface';
...
export class MediaService implements MediaProcessor {
  ...
  checkFfmpegAvailability(filePath: string): void {
    ...
    if (!this.ffmpegInstalled) {
      throw MediaException.ffmpegNotInstalled();
    }
  }
  ...
  async generateScreenshotAtTimestamp(
    ...
    try {
      ...
      try {
        ...
        throw MediaException.failedThumbnailGeneration(fallbackError);
      }
    }
  }
